...
RUN mkdir -p /gppreport/bin ; mkdir -p /gppreport/reports ; mkdir -p /gppreport/queries

ADD queries/*.sql /gppreport/queries/

ADD main /gppreport/main
#ENTRYPOINT /gppreport/bin/main

ENV PKG_CONFIG_PATH "/oreclient_install_dir/instantclient_12_2"
ENV LD_LIBRARY_PATH "/oreclient_install_dir/instantclient_12_2"
ENV ORACLE_USER "GPPSP"
ENV ORACLE_PASSWORD "firstpass#234"
ENV ORACLE_HOST "10.182.237.53"
ENV ORACLE_SERVICE "FNDSIT01"
ENV ORACLE_PORT "1623"
